...
export default function Encomiendas() {
  ...
  const openEditModal = (encomienda) =>
    setEditModal({ open: true, encomienda });
  ...
  const filteredEncomiendas = encomiendas.filter((item) => {
    ...
    const matchesSearch =
      ...
      (item.clienteRemitente?.nombreCompleto &&
        item.clienteRemitente.nombreCompleto.toLowerCase().includes(term)) ||
      (item.clienteConsignatario?.nombreCompleto &&
        item.clienteConsignatario.nombreCompleto
          .toLowerCase()
          .includes(term)) ||
  ...
  function getFechaEntregaBolivia() {
    const now = new Date();
...
function coincideDestino(destino) {
  ...
  async function handleEntrega(id, setSnackbar) {
    try {
      ...
      // setRows(prev =>
      //   prev.map(row =>
      //     row.id === id ? { ...row, fechaEntrega: data.fechaEntrega } : row
      //   )
      // )
    } catch (error) {
    ...
    <Box className="encomiendas-view" sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        ...
        <TableContainer className="encomiendas-table-container">
          <Table
            className="encomiendas-table"
            aria-label="tabla de encomiendas"
          >
            <TableHead>
              <TableRow>
...
>
  {row.fechaEntrega ? (
    ...
    //  Mostrar "Anulado" si estado es false
    "Anulado"
  ) : !coincideDestino(row.destino)? (
    //  No mostrar nada si destino es true
    null
  ) : (
    // Mostrar botón si fechaEntrega es null, estado true y destino treu
    <Button
      variant="contained"
      size="small"
      color="primary"
      onClick={() => {
        row.fechaEntrega = getFechaEntregaBolivia()
        handleEntrega(row.id)
      }}
    >
      Entregar
    </Button>
...
>
  <span>
    <IconButton
      size="small"
      color="info"
      disabled={row.envio != null || row.estado === false || row.fechaEntrega}
...
>
  <span>
    <IconButton
      size="small"
      color="error"
      disabled={!row.estado || row.fechaEntrega}
    ...
    >
      ...
      >
        ...
        <DialogContent>
          ...
          >
            ...
            <strong>{confirmDelete.encomienda?.numero}</strong> (
            {confirmDelete.encomienda?.contenido}). Esta acción registrará la
            encomienda como <strong>Anulada</strong> en la base de datos.
          </DialogContentText>
        </DialogContent>
      ...
      >
        ...
        <DialogContent dividers className="ticket-print-area">
          {printModal.encomienda && (
            <Box sx={{ p: 1 }}>
              ...
              <Typography variant="body2" sx={{ mb: 0.5 }}>
                ...
                {printModal.encomienda.clienteRemitente?.nombreCompleto} (
                {printModal.encomienda.clienteRemitente?.telefono})
              </Typography>
              <Typography variant="body2" sx={{ mb: 0.5 }}>
                <strong>Consignatario:</strong>{" "}
                {printModal.encomienda.clienteConsignatario?.nombreCompleto} (
                {printModal.encomienda.clienteConsignatario?.telefono})
              </Typography>
              <Typography variant="body2" sx={{ mb: 0.5 }}>
                <strong>Fecha Recepción:</strong>{" "}
                {printModal.encomienda.fechaRecepcion}
              </Typography>
              <Typography variant="body2" sx={{ mb: 0.5 }}>
                <strong>Registrado por:</strong>{" "}
                {printModal.encomienda.usuario?.usuario}
              </Typography>
